...
const choose_insect_btns =
  document.querySelectorAll<HTMLButtonElement>('.choose-insect-btn')
...
function createInsect() {
  ...
  const insectObj: InsectObj = selected_insect
  const insect = document.createElement('div')
  insect.classList.add('insect')
  const { x, y } = getRandomLocation()
  insect.style.top = `${y}px`
  insect.style.left = `${x}px`
  insect.innerHTML = `<img src="${insectObj.src}" alt="${
    insectObj.alt
...
function catchInsect(this: HTMLDivElement) {
  increaseScore()
